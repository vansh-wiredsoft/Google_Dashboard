...
export default function KpiForm({ mode, role = "superadmin" }) {
  ...
  useEffect(() => {
    if (role === "admin") {
      setForm((current) => ({ ...current, companyId: getCompanyId() }));
    }
  }, [role]);
  ...
  const handleSave = async () => {
    ...
    if (role === "superadmin" && !form.companyId) {
    ...
    try {
      const payload = {
        ...
        companyId: role === "superadmin" ? form.companyId : getCompanyId(),
      };
  ...
  return (
    <Layout role={role} title={pageTitle}>
      <Paper elevation={0} sx={{ p: { xs: 2, sm: 3 }, borderRadius: 3, border: "1px solid", borderColor: "divider", bgcolor: getSurfaceBackground(theme) }}>
        ...
        <Stack spacing={2}>
          {role === "superadmin" && (
            <TextField label="Company" value={form.companyId} onChange={(event) => setForm((current) => ({ ...current, companyId: event.target.value }))} select fullWidth>
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
            <TextField label="Theme" value={form.themeKey} onChange={(event) => setForm((current) => ({ ...current, themeKey: event.target.value }))} select fullWidth>
            ...
            <TextField label="KPI Name" value={form.displayName} onChange={(event) => setForm((current) => ({ ...current, displayName: event.target.value }))} fullWidth />
          </Stack>
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
            <TextField label="Domain Category" value={form.domainCategory} onChange={(event) => setForm((current) => ({ ...current, domainCategory: event.target.value }))} fullWidth />
            <TextField label="WI Weight" type="number" inputProps={{ min: 0 }} value={form.wiWeight} onChange={(event) => setForm((current) => ({ ...current, wiWeight: event.target.value }))} fullWidth />
          </Stack>
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
            <TextField label="Start Date" type="date" value={form.startDate} onChange={(event) => setForm((current) => ({ ...current, startDate: event.target.value }))} InputLabelProps={{ shrink: true }} fullWidth />
            <TextField label="End Date" type="date" value={form.endDate} onChange={(event) => setForm((current) => ({ ...current, endDate: event.target.value }))} InputLabelProps={{ shrink: true }} fullWidth />
          </Stack>
          {mode === "edit" && (
            <FormControlLabel control={<Switch checked={form.isActive} onChange={(event) => setForm((current) => ({ ...current, isActive: event.target.checked }))} />} label="Active" />
          )}
        </Stack>
